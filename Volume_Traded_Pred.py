...
# 2. Top Traded Stocks in the Past Month
    ...
    st.markdown("Displays the daily volume traded over the past 3 months for selected major stocks.")
    ...
    for ticker in tickers:
        ...
        if not df.empty:
            ax.plot(df.index, df['Volume'] / 1e6, label=ticker)  # scale volume to millions
            df = df[['Volume']].copy()
            df['Ticker'] = ticker
            volume_table = pd.concat([volume_table, df])

    ax.set_title("Volume Traded Over the Past 3 Months")
    ax.set_xlabel("Date")
    ax.set_ylabel("Volume (Millions)")
    ax.legend()
    st.pyplot(fig)

    st.subheader("\U0001F4C9 Last 20 Records of Volume Traded")
    st.dataframe(volume_table.tail(20))
