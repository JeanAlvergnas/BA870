...
if page == "Top Traded Stocks":
    ...
    for ticker in tickers:
        try:
            ...
            if not df.empty and 'Volume' in df.columns:
                ...
                avg_volumes.append((ticker, avg_vol))
    ...
    filtered_data = [df for df in volume_data if df['Ticker'].iloc[0] in top3_tickers]

    if filtered_data:
        combined_df = pd.concat(filtered_data, ignore_index=True)
        combined_df = combined_df.groupby(['Date', 'Ticker'], as_index=False).agg({'Volume': 'mean'})
